...
namespace Albedo
{
    public static class AlbedoUtils
    {
            ...
            float homingVelocity, float N)
        {
            ...
            if (_defExtraUpdates == -1) _defExtraUpdates = projectile.extraUpdates;
        ...
        {
            var vector = new Vector2(rand.NextFloat(0f - directionMult, directionMult),
                rand.NextFloat(0f - directionMult, directionMult));
            while (vector.X == 0f && vector.Y == 0f)
                vector = new Vector2(rand.NextFloat(0f - directionMult, directionMult),
                    rand.NextFloat(0f - directionMult, directionMult));
            vector.Normalize();
        ...
        public static bool LiveRarity(int rarity, DrawableTooltipLine line)
        {
            if (line.mod == "Terraria" && line.Name == "ItemName")
            {
                ...
                foreach (var item1 in item)
                    GameShaders.Armor.Apply(GameShaders.Armor.GetShaderIdFromItemId(rarity), item1);
        ...
        public static void NewDust(Projectile projectile, Vector2 speed, int type, int count = 1, int chance = 100,
        ...
        public static NPC NpcExists(int whoAmI, params int[] types)
        {
            if (whoAmI <= -1 || whoAmI >= 200 || !npc[whoAmI].active ||
                (types.Length != 0 && !types.Contains(npc[whoAmI].type))) return null;
            return npc[whoAmI];
        }
        ...
        public static Projectile ProjectileExists(int whoAmI, params int[] types)
        {
            if (whoAmI <= -1 || whoAmI >= 1000 || !projectile[whoAmI].active ||
                (types.Length != 0 && !types.Contains(projectile[whoAmI].type))) return null;
        ...
        public static int GetByUuidReal(int player, int projectileIdentity, params int[] projectileType)
        {
            for (var i = 0; i < 1000; i++)
                if (projectile[i].active && projectile[i].identity == projectileIdentity &&
                    ...
                    (projectileType.Length == 0 || projectileType.Contains(projectile[i].type)))
                    return i;
            return -1;
        }
        ...
        private static void Chat(string message, byte colorR = byte.MaxValue, byte colorG = byte.MaxValue,
            byte colorB = byte.MaxValue, bool sync = true)
        {
            switch (netMode)
            {
                case NetmodeID.SinglePlayer:
                case NetmodeID.MultiplayerClient:
                    NewText(message, colorR, colorG, colorB);
                    break;
                default:
                {
