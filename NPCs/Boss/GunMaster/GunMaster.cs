...
using System.IO;
using Albedo.Global;
using Albedo.Projectiles.Boss.GunMaster;
using Terraria;
using Terraria.ID;
...
namespace Albedo.NPCs.Boss.GunMaster
{
    [AutoloadBossHead]
    public class GunMaster : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            //bossBag = ModContent.ItemType<GunMasterBag>();
        }
        ...
        {
            AlbedoGlobalNpc.GunMaster = npc.whoAmI;
            switch (npc.localAI[3])
            {
                ...
                case 1f:
                    Aura(2000f, BuffID.WaterCandle, reverse: true, 86, checkDuration: false, targetEveryone: false);
                    break;
            }
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
				...
				if (AlbedoUtils.ProjectileExists(_spriteProj, ModContent.ProjectileType<GunMasterBoss>()) == null)
				{
					if (Main.netMode == 0)
					{
						int num = 0;
						...
						if (num >= 0 && Main.netMode == NetmodeID.SinglePlayer)
						{
							Projectile obj = Main.projectile[num];
							obj.SetDefaults(ModContent.ProjectileType<GunMasterBoss>());
					...
					{
						_spriteProj = Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<GunMasterBoss>(), 0, 0f, Main.myPlayer, 0f, npc.whoAmI);
					}
				}
            }
            ...
            switch ((int)npc.ai[0])
            {
                ...
                case 9:
                    ...
                    if ((npc.ai[1] += 1f) == 1f)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile.NewProjectile(npc.Center, npc.ai[3].ToRotationVector2(), ModContent.ProjectileType<GunMasterDeathraySmall>(), 0, 0f, Main.myPlayer);
                            Projectile.NewProjectile(npc.Center, -npc.ai[3].ToRotationVector2(), ModContent.ProjectileType<GunMasterDeathraySmall>(), 0, 0f, Main.myPlayer);
                        }
                    }
                ...
                case 11:
                    ...
                    if ((npc.ai[3] += 1f) > 5f)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile.NewProjectile(npc.Center, Utils.RotatedBy(Vector2.UnitY, (double)MathHelper.ToRadians(20f) * (Main.rand.NextDouble() - 0.5), default(Vector2)), ModContent.ProjectileType<GunMasterDeathrayMark>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num12);
                            Projectile.NewProjectile(npc.Center, -Utils.RotatedBy(Vector2.UnitY, (double)MathHelper.ToRadians(20f) * (Main.rand.NextDouble() - 0.5), default(Vector2)), ModContent.ProjectileType<GunMasterDeathrayMark>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num12);
                        }
                    }
                ...
                case 13:
                    ...
                    if ((npc.ai[3] += 1f) > 5f)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Projectile.NewProjectile(npc.Center, Utils.RotatedBy(Vector2.UnitY, (double)MathHelper.ToRadians(20f) * (Main.rand.NextDouble() - 0.5), default(Vector2)), ModContent.ProjectileType<GunMasterDeathrayMark>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num33);
                            Projectile.NewProjectile(npc.Center, -Utils.RotatedBy(Vector2.UnitY, (double)MathHelper.ToRadians(20f) * (Main.rand.NextDouble() - 0.5), default(Vector2)), ModContent.ProjectileType<GunMasterDeathrayMark>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num33);
                        }
                    }
                    if ((npc.ai[1] += 1f) > 2400f / Math.Abs(npc.velocity.X))
                ...
                case 15:
                {
                    ...
                    else if (npc.ai[1] == 60f && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center, vector2, ModContent.ProjectileType<GunMasterSword>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num15, (float)npc.whoAmI);
                    }
                    break;
                }
                case 16:
                ...
                case 19:
                    ...
                    if (npc.ai[1] == 0f && Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center, Vector2.UnitX * (0f - num30), ModContent.ProjectileType<GunMasterSword>(), npc.damage * 3 / 8, 0f, Main.myPlayer, num32, (float)npc.whoAmI);
                    }
                    if ((npc.ai[1] += 1f) > 60f)
                    {
                        npc.netUpdate = true;
        ...
        {
            AlbedoWorld.DownedGunMaster = true;
            if (Main.netMode == NetmodeID.Server)
            {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(texture, npc.Center - Main.screenPosition + new Vector2(0f, npc.gfxOffY), frame, npc.GetAlpha(lightColor), npc.rotation, origin, this.npc.scale, effects, 0f);
            return false;
        }
    }
}