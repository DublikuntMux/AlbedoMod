    ...
    public class MiniGun : ModNPC
    {
                ...
                if (npc.ai[1] > 120f)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                        for (var i = 0; i < 5; i++)
                        {
                            ...
                            var num = Projectile.NewProjectile(npc.Center, vector, ModContent.ProjectileType<Laser>(),
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (npc.life <= 0)
                for (var j = 0; j < 30; j++)
                {
                    var num2 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 2.5f);
