...
namespace Albedo.NPCs.Boss.GunGod
{
	...
	public class GunGod : ModNPC
	{
		...
		public override void AI()
		{
			...
			switch ((int) npc.ai[0]) {
				case -3: {
					...
					for (int i = 0; i < 5; i++) {
						int num3 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 2.5f);
				...
				case -2:
					if (AliveCheck(val)) {
						...
						for (int num52 = 0; num52 < 5; num52++) {
							int num53 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 2.5f);
				...
				case -1: {
					...
					if ((npc.ai[1] += 1f) > 120f) {
						for (int num59 = 0; num59 < 5; num59++) {
							int num60 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 1.5f);
				...
				case 2: {
					...
					if (npc.localAI[3] > 1f) {
						...
						for (int m = 0; m < 128; m++) {
							...
							int num21 = Dust.NewDust(npc.Center, 0, 0, 87);
				...
				case 3:
					...
					if (npc.localAI[3] > 1f)
						for (int num54 = 0; num54 < 2; num54++) {
							int num55 = Dust.NewDust(npc.Center - npc.velocity * Main.rand.NextFloat(), 0, 0, 87);
					...
					if (npc.ai[2] == 0f) {
						...
						if (Main.netMode != NetmodeID.MultiplayerClient) {
							...
							for (int num37 = -1; num37 <= 1; num37 += 2) {
								int num38 = Projectile.NewProjectile(npc.Center + 3000 * num36 * Vector2.UnitX,
						...
						for (int num39 = 0; num39 < 30; num39++) {
							int num40 = Dust.NewDust(npc.position, npc.width, npc.height, 76);
				...
				case 7: {
					...
					for (int num45 = 0; num45 < 5; num45++) {
						int num46 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 1.5f);
					...
					if ((npc.ai[1] += 1f) == 1f) {
						...
						for (int n = -1; n <= 1; n += 2) {
							...
							for (int num23 = 0; num23 < 20; num23++) {
								int num24 = Dust.NewDust(npc.Center, 0, 0, 31, vector4.X, vector4.Y, 0, default, 3f);
							...
							for (int num25 = 1; num25 <= 14; num25++) {
								...
								for (int num28 = 0; num28 < 3; num28++) {
									int num29 = Dust.NewDust(npc.Center, 0, 0, 70, vector5.X, vector5.Y, 0, default,
										3f);
				...
				case 15: {
					...
					if (npc.ai[1] == 0f && npc.ai[2] != 2f && Main.netMode != NetmodeID.MultiplayerClient) {
						...
						int num14 = Projectile.NewProjectile(npc.Center, Vector2.Zero,
			...
			if (npc.ai[0] >= 9f && npc.dontTakeDamage)
				for (int num62 = 0; num62 < 5; num62++) {
					int num63 = Dust.NewDust(npc.position, npc.width, npc.height, 87, 0f, 0f, 0, default, 1.5f);
			...
			{
				float num64 = 0f;
				for (int num65 = 0; num65 < repeats; num65++) num64 = MathHelper.Lerp(num64, 1f, 0.08f);
				float num66 = 1400f * (1f - num64);
				float num67 = (float) Math.PI * 2f * num64;
				for (int num68 = 0; num68 < 4; num68++) {
					int num69 = Dust.NewDust(
		...
		public override void BossLoot(ref string name, ref int potionType) => potionType = 3544;
