...
namespace Albedo.NPCs.Boss.HellGuard
{
    public class HellGuardHand : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.lavaImmune = true;
            npc.aiStyle = -1;
            for (var index = 0; index < npc.buffImmune.Length; ++index)
        ...
        {
            var val = AlbedoUtils.NpcExists(npc.ai[2], ModContent.NPCType<HellGuard>());
            if (val == null)
            {
                npc.life = 0;
                npc.checkDead();
            ...
            var val2 = Main.player[npc.target];
            ...
            switch ((int) npc.ai[0])
            {
                case -1:
                    ...
                    {
                        var num = (float) Math.PI / 4f * Main.rand.NextFloat();
                        for (var i = 0; i < 8; i++)
                        {
                            var num2 = num + (float) Math.PI / 4f * (i + Main.rand.NextFloat(-0.5f, 0.5f));
                            Projectile.NewProjectile(npc.Center, Vector2.Zero,
                                ModContent.ProjectileType<EarthChainBlast2>(), 0, 0f, Main.myPlayer, num2, 3f);
                        }
                    }
                ...
                case 1:
                    ...
                    else if (npc.ai[1] < 75f)
                    {
                        ...
                        if (npc.ai[3] < 0f && vector.X < val2.Center.X + 400f) vector.X = val2.Center.X + 400f;
                        if (npc.ai[3] > 0f && vector.X > val2.Center.X - 400f) vector.X = val2.Center.X - 400f;
                        if (npc.Distance(vector) > 50f) Movement(vector, val.localAI[2] == 1f ? 2.4f : 1.2f, 32f);
                        if (val.localAI[2] == 1f)
                        {
                            var npc2 = npc;
                    ...
                    else
                    {
                        ...
                        for (var j = 0; j < 5; j++)
                        {
                            var num3 = Dust.NewDust(npc.position, npc.width, npc.height, 6,
                                (0f - npc.velocity.X) * 0.25f, (0f - npc.velocity.Y) * 0.25f, 0, default, 3f);
                        ...
                        if (((npc.localAI[1] += 1f) > 60f && npc.Distance(val2.Center) > 1000f) || (npc.ai[3] > 0f
                                ? npc.Center.X > Math.Min(val.Center.X, val2.Center.X) + 300f
                                : npc.Center.X < Math.Max(val.Center.X, val2.Center.X) - 300f))
                        {
                            npc.ai[1] = val.localAI[2] == 1f ? 15 : 0;
                            npc.localAI[1] = 0f;
                            npc.netUpdate = true;
                            if (val.localAI[2] == 1f && Main.netMode != NetmodeID.MultiplayerClient)
                                Projectile.NewProjectile(npc.Center + Vector2.Normalize(npc.velocity) * 100f,
                                    Vector2.Zero, ModContent.ProjectileType<EarthChainBlast>(), npc.damage / 4, 0f,
                                    Main.myPlayer, npc.velocity.ToRotation(), 7f);
                ...
                case 3:
                    if (npc.ai[3] > 0f)
                    {
                        ...
                        if (npc.Distance(vector) > 50f) Movement(vector, 0.4f, 32f);
                    }
                    else
                    {
                        vector = val2.Center;
                    ...
                    if ((npc.localAI[0] += 1f) > (val.localAI[2] == 1f ? 18 : 24))
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(npc.Center, Vector2.UnitX * npc.ai[3],
                                ModContent.ProjectileType<FlowerPetal>(), npc.damage / 4, 0f, Main.myPlayer, 1f);
                    }
                ...
                case 7:
                {
                    ...
                    if (npc.ai[3] > 0f)
                        ...
                        npc.rotation = (float) Math.PI / 2f;
                    if (npc.position.Y + npc.height > npc.localAI[0]) npc.noTileCollide = false;
                    ...
                    if (npc.localAI[0] != 0f)
                    {
                        ...
                        if (Main.netMode != 1)
                        {
                            ...
                            Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<FuseBomb>(),
                                npc.damage / 4, 0f, Main.myPlayer);
                            if (val.localAI[2] == 1f)
                            {
                                for (var k = 0; k < 4; k++)
                                {
                                    var vector2 = Vector2.Normalize(npc.oldVelocity)
                                        .RotatedBy(Math.PI / 2.0 * (npc.ai[3] < 0f ? k : k + 0.5));
                                    Projectile.NewProjectile(npc.Center, 1.5f * vector2,
                                        ModContent.ProjectileType<LavaPalladOrb>(), npc.damage / 4, 0f, Main.myPlayer);
                                }
                            }
                            else
                            {
                                ...
                                for (var l = 0; l <= 3; l++)
                                {
                                    var num5 = (int) center.X / 16 + 250 * l / 16 * (int) (0f - npc.ai[3]);
                                    var num6 = (int) center.Y / 16;
                                    Projectile.NewProjectile(num5 * 16 + 8, num6 * 16 + 8, 0f, 0f,
                                        ModContent.ProjectileType<LavaGeyser>(), npc.damage / 4, 0f, Main.myPlayer,
                                        npc.whoAmI);
                                }
                            }
                        }
                    ...
                    npc.localAI[1] += 1f;
                    if (!(npc.localAI[1] > (val.localAI[2] == 1f ? 20 : 30))) break;
                    npc.netUpdate = true;
                    npc.ai[0] += 1f;
                    npc.ai[1] = 0f;
                    npc.localAI[0] = 0f;
                    ...
                    for (var m = 0; m < 200; m++)
                        if (Main.npc[m].active && Main.npc[m].type == npc.type && m != npc.whoAmI &&
                            Main.npc[m].ai[2] == npc.ai[2])
                        {
                ...
                case 8:
                    npc.noTileCollide = true;
                    vector = val.Center + Vector2.UnitX * (0f - npc.ai[3]) * ((val.localAI[2] == 1f ? 450 : 600) +
                                                                              500f * (1f - Math.Min(1f,
                                                                                  npc.localAI[1] / 240f)));
                    if (npc.Distance(vector) > 25f) Movement(vector, 0.8f, 32f);
                    npc.rotation = (float) Math.PI / 2f * (0f - npc.ai[3]) + (float) Math.PI;
                    if ((npc.localAI[1] += 1f) > 90f && npc.localAI[1] % 6f == 0f && Main.netMode != 1)
                    {
                        var num4 = Projectile.NewProjectile(npc.Center + Vector2.UnitY * Main.rand.NextFloat(50f, 100f),
                            Vector2.Zero, ModContent.ProjectileType<MoonLordSunBlast>(), npc.damage / 4, 0f,
                            Main.myPlayer, Vector2.UnitY.ToRotation(), 16f);
                        if (num4 != 1000) Main.projectile[num4].localAI[0] = 2f;
                    }
                ...
                case 10:
                    ...
                    if (npc.ai[3] > 0f)
                    {
                        ...
                        if (npc.Distance(vector) > 50f) Movement(vector, 0.6f, 32f);
                        npc.rotation = (float) Math.PI / 2f;
                    }
                    else
                    {
                    ...
                    if ((npc.localAI[0] += 1f) > 60f && npc.ai[1] > 120f)
                    {
                        npc.localAI[0] = 0f;
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            Projectile.NewProjectile(npc.Center, Vector2.UnitY * 2f,
                                ModContent.ProjectileType<CrystalBomb>(), npc.damage / 4, 0f, Main.myPlayer,
                                val2.position.Y);
                    }
        ...
        {
            npc.frame.Y = npc.localAI[3] == 1.0 ? 0 : frameHeight;
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (var i = 0; i < NPCID.Sets.TrailCacheLength[npc.type]; i++)
            {
            ...
            var texture2 = ModContent.GetTexture("Albedo/NPCs/Boss/HellGuard/HellGuardHand_Glow");
            if (npc.dontTakeDamage)
            {
                var vector2 = Vector2.UnitX * Main.rand.NextFloat(-180f, 180f);
                Main.spriteBatch.Draw(texture,
                    npc.Center + vector2 - Main.screenPosition + new Vector2(0f, npc.gfxOffY), frame,
                    npc.GetAlpha(lightColor) * 0.5f, npc.rotation, origin, npc.scale, effects, 0f);
            ...
            Main.spriteBatch.Draw(texture2, npc.Center - Main.screenPosition + new Vector2(0f, npc.gfxOffY), frame,
                Color.White, npc.rotation, origin, npc.scale, effects, 0f);
            return false;
        }
    }
}