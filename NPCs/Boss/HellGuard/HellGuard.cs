...
namespace Albedo.NPCs.Boss.HellGuard
{
	...
	public class HellGuard : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/HellGuard");
		}
		...
		public override void AI()
		{
			...
			if (npc.localAI[3] == 0.0) {
				...
				int num1 = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, ModContent.NPCType<HellGuardHand>(),
				...
				int num2 = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, ModContent.NPCType<HellGuardHand>(),
					npc.whoAmI, 0.0f, 0.0f, npc.whoAmI, -1f);
				...
				switch (npc.ai[0]) {
					case -1f:
						...
						if (npc.ai[1] == 120.0) {
							...
							for (int index = 0; index < 8; ++index) {
								float num4 = num3 +
								             (float) (0.78 * (index + (double) Main.rand.NextFloat(-0.5f, 0.5f)));
						...
						for (int index1 = 0; index1 < 5; ++index1) {
							int index2 = Dust.NewDust(npc.Center, 0, 0, DustID.InfernoFork, 0.0f, 0.0f, 0, new Color(),
								1.5f);
								...
								for (int index = -1; index <= 1; ++index)
									Projectile.NewProjectile(npc.Center + Vector2.UnitY * 60f,
										(npc.localAI[2] == 1f ? 12 : 8) * npc.DirectionTo(player.Center)
											.RotatedBy(MathHelper.ToRadians(8 * index)), ProjectileID.Fireball,
										npc.damage / 4, 0f,
										Main.myPlayer);
						}
