...
namespace Albedo.NPCs.Boss.HellGuard
{
    ...
    public class HellGuard : ModNPC
    {
            ...
            if (npc.localAI[3] == 0.0)
            {
                ...
                if (npc.ai[1] == 0.0)
                {
                    npc.Center = Main.player[npc.target].Center +
                                 new Vector2(500 * Math.Sign(npc.Center.X - Main.player[npc.target].Center.X), -250f);
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        Projectile.NewProjectile(npc.Center + Vector2.UnitY * 1000f, Vector2.Zero,
                            ModContent.ProjectileType<EarthChainBlast2>(), 0, 0f, Main.myPlayer,
                            0f - Vector2.UnitY.ToRotation(), 10f);
                        Projectile.NewProjectile(npc.Center - Vector2.UnitY * 1000f, Vector2.Zero,
                            ModContent.ProjectileType<EarthChainBlast2>(), 0, 0f, Main.myPlayer,
                            Vector2.UnitY.ToRotation(), 10f);
                    }
                }
                ...
                {
                    var num = (float) (0.78 * (index + (double) Main.rand.NextFloat(-0.5f, 0.5f)));
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<EarthChainBlast2>(), 0,
                        0.0f, Main.myPlayer, num, 3f);
                }

                if (Main.netMode != NetmodeID.MultiplayerClient)
                    Projectile.NewProjectile(npc.Center, Vector2.Zero, ModContent.ProjectileType<Arena>(),
                        npc.damage / 4, 0f, Main.myPlayer, 0f, npc.whoAmI);
                var num1 = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, ModContent.NPCType<HellGuardHand>(),
                    npc.whoAmI, 0.0f, 0.0f, npc.whoAmI, 1f);
                if (num1 < 200 && Main.netMode == NetmodeID.Server)
                    NetMessage.SendData(MessageID.SyncNPC, -1, -1, null, num1);
                var num2 = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, ModContent.NPCType<HellGuardHand>(),
                    npc.whoAmI, 0.0f, 0.0f, npc.whoAmI, -1f);
                ...
                switch (npc.ai[0])
                {
                    case -1f:
                        ...
                        if (npc.ai[1] == 120.0)
                        {
                            ...
                            if (Main.netMode == NetmodeID.MultiplayerClient)
                                break;
                            var num3 = 0.7853982f * Main.rand.NextFloat();
                            for (var index = 0; index < 8; ++index)
                            {
                                var num4 = num3 + (float) (0.78 * (index + (double) Main.rand.NextFloat(-0.5f, 0.5f)));
                                Projectile.NewProjectile(npc.Center, Vector2.Zero,
                                    ModContent.ProjectileType<EarthChainBlast2>(), 0, 0.0f, Main.myPlayer, num4, 3f);
                            }
                        ...
                        var npc2 = npc;
                        npc2.velocity *= 0.9f;
                        var num = (int) (npc.lifeMax / 2 / 120 * (double) Main.rand.NextFloat(1f, 1.5f));
                        npc.life += num;
                        ...
                        {
                            var index2 = Dust.NewDust(npc.Center, 0, 0, 174, 0.0f, 0.0f, 0, new Color(), 1.5f);
                        ...
                        break;
                    case 0.0f:
                        if (!player.active || player.dead || Vector2.Distance(npc.Center, player.Center) > 2500.0 ||
                        ...
                        var center1 = player.Center;
                        center1.Y -= 325f;
                        if (npc.Distance(center1) > 50.0)
                        ...
                        for (var index = 0; index < 200; ++index)
                            if (Main.npc[index].active && Main.npc[index].type == ModContent.NPCType<HellGuardHand>() &&
                                Main.npc[index].ai[2] == (double) npc.whoAmI)
                            {
                    ...
                    case 1f:
                        ...
                        var center2 = player.Center;
                        for (var index = 0; index < 22; ++index)
                        {
                            center2.Y -= 16f;
                            var tileSafely = Framing.GetTileSafely(center2);
                            if (tileSafely.active() && !tileSafely.inActive() && Main.tileSolid[tileSafely.type] &&
        ...
        public override void NPCLoot()
        {
            ...
            if (Main.rand.Next(0, 100) <= 10) Item.NewItem(npc.Hitbox, ModContent.ItemType<HellGuardTrophy>());
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            var texture2 = ModContent.GetTexture("Albedo/NPCs/Boss/HellGuard/HellGuard_Glow");
            ...
            Main.spriteBatch.Draw(texture2, npc.Center - Main.screenPosition + new Vector2(0f, npc.gfxOffY), frame,
                Color.White * npc.Opacity, npc.rotation, origin, npc.scale, effects, 0f);
            return false;
        }
    }
}