...
namespace Albedo.Projectiles.Weapons.Ranged
{
	...
	{
		public override void SetStaticDefaults() => DisplayName.SetDefault("Fishron RPG");
		...
		public override void AI()
		{
			if (projectile.ai[1] == 0f) {
				for (int i = 0; i < 5; i++) {
					int num = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
			...
			if (projectile.owner == Main.myPlayer && projectile.timeLeft <= 3) {
				...
				for (int j = 0; j < 2; j++) {
					...
					int num4 = Dust.NewDust(
						new Vector2(projectile.position.X + 3f + num2, projectile.position.Y + 3f + num3) -
						projectile.velocity * 0.5f, projectile.width - 8, projectile.height - 8, 33, 0f, 0f, 100);
					var obj2 = Main.dust[num4];
					obj2.scale *= 2f + Main.rand.Next(10) * 0.1f;
					...
					num4 = Dust.NewDust(
						...
						projectile.velocity * 0.5f, projectile.width - 8, projectile.height - 8, 33, 0f, 0f, 100,
						default, 0.5f);
					Main.dust[num4].fadeIn = 1f + Main.rand.Next(5) * 0.1f;
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int j = 0; j < 20; j++) {
				int num2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
