...
namespace Albedo.Projectiles.Combined
{
	public class CosmoBulletProjectile : ModProjectile
	{
		...
		private void OnHit()
		{
			...
			for (int i = 0; i < 5; i++) {
				int num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 68);
				Main.dust[num].noGravity = true;
				var obj = Main.dust[num];
				obj.velocity *= 1.5f;
			...
			for (int j = 0; j < 3; j++) {
				float num2 = (0f - projectile.velocity.X) * Main.rand.Next(40, 70) * 0.01f +
				             Main.rand.Next(-20, 21) * 0.4f;
				float num3 = (0f - projectile.velocity.Y) * Main.rand.Next(40, 70) * 0.01f +
				             Main.rand.Next(-20, 21) * 0.4f;
			...
			for (int l = 0; l < 3; l++) {
				int num4 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0f, 0f, 100,
					default, 2.5f);
		...
		public override void Kill(int timeleft)
		{
			...
			Main.PlaySound(SoundID.Item10, projectile.position);
			for (int i = 0; i < 10; i++) {
				int num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 171, 0f, 0f, 100);
			...
			for (int j = 0; j < 10; j++) {
				...
				int num3 = Dust.NewDust(projectile.position, projectile.width, projectile.height, num2,
