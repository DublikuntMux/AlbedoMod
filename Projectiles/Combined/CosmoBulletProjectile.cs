...
namespace Albedo.Projectiles.Combined
{
    public class CosmoBulletProjectile : ModProjectile
    {
        ...
        {
            for (var i = 0; i < 7; i++) AlbedoUtils.NewDust(projectile, Vector2.Zero, _dusts[_currentDust]);
            _currentDust++;
            if (_currentDust > 5) _currentDust = 0;
            var num2 = (float) Math.Sqrt(projectile.velocity.X * projectile.velocity.X +
                                         projectile.velocity.Y * projectile.velocity.Y);
            var num3 = projectile.localAI[0];
            if (num3 == 0f)
            {
            ...
            var num4 = projectile.position.X;
            var num5 = projectile.position.Y;
            var num6 = 300f;
            var flag = false;
            var num7 = 0;
            if (projectile.ai[1] == 0f)
            {
                for (var j = 0; j < 200; j++)
                    ...
                    {
                        var num8 = Main.npc[j].position.X + Main.npc[j].width / 2f;
                        var num9 = Main.npc[j].position.Y + Main.npc[j].height / 2f;
                        var num10 = Math.Abs(projectile.position.X + projectile.width / 2f - num8) +
                ...
                if (flag) projectile.ai[1] = num7 + 1;
                flag = false;
            }

            if (projectile.ai[1] > 0f)
            {
                var num11 = (int) (projectile.ai[1] - 1f);
                if (Main.npc[num11].active && Main.npc[num11].CanBeChasedBy(projectile, true) &&
                    !Main.npc[num11].dontTakeDamage)
                {
                    var num12 = Main.npc[num11].position.X + Main.npc[num11].width / 2;
                    var num13 = Main.npc[num11].position.Y + Main.npc[num11].height / 2;
            ...
            if (!projectile.friendly) flag = false;
            if (flag)
            {
                var num14 = num3;
                var vector = new Vector2(projectile.position.X + projectile.width * 0.5f,
                    projectile.position.Y + projectile.height * 0.5f);
                var num15 = num4 - vector.X;
                var num16 = num5 - vector.Y;
                var num17 = (float) Math.Sqrt(num15 * num15 + num16 * num16);
                num17 = num14 / num17;
                num15 *= num17;
                num16 *= num17;
                var num18 = 8;
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            target.AddBuff(31, Main.rand.NextBool(3) ? 180 : 60);
        ...
        private void OnHit()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                var num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 68);
            ...
            for (var j = 0; j < 3; j++)
            {
                var num2 = (0f - projectile.velocity.X) * Main.rand.Next(40, 70) * 0.01f +
                           Main.rand.Next(-20, 21) * 0.4f;
                var num3 = (0f - projectile.velocity.Y) * Main.rand.Next(40, 70) * 0.01f +
                           Main.rand.Next(-20, 21) * 0.4f;
                Projectile.NewProjectile(projectile.position.X + num2, projectile.position.Y + num3, num2, num3,
                    ProjectileID.CrystalShard, projectile.damage, 0f, projectile.owner);
            }
            ...
            for (var l = 0; l < 3; l++)
            {
                var num4 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0f, 0f, 100,
                    default, 2.5f);
                Main.dust[num4].noGravity = true;
                var obj3 = Main.dust[num4];
                obj3.velocity *= 3f;
                num4 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0f, 0f, 100, default,
                    1.5f);
                var obj4 = Main.dust[num4];
                obj4.velocity *= 2f;
            }
        ...
        public override void Kill(int timeleft)
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                var num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 171, 0f, 0f, 100);
            ...
            for (var j = 0; j < 10; j++)
            {
                var num2 = Main.rand.Next(139, 143);
                var num3 = Dust.NewDust(projectile.position, projectile.width, projectile.height, num2,
                    (0f - projectile.velocity.X) * 0.3f, (0f - projectile.velocity.Y) * 0.3f, 0, default, 1.2f);
                var val = Main.dust[num3];
                val.velocity.X += Main.rand.Next(-50, 51) * 0.01f;
                var val2 = Main.dust[num3];
                val2.velocity.Y += Main.rand.Next(-50, 51) * 0.01f;
                var val3 = Main.dust[num3];
                val3.velocity.X *= 1f + Main.rand.Next(-50, 51) * 0.01f;
                var val4 = Main.dust[num3];
                val4.velocity.Y *= 1f + Main.rand.Next(-50, 51) * 0.01f;
                var val5 = Main.dust[num3];
                val5.velocity.X += Main.rand.Next(-50, 51) * 0.05f;
                var val6 = Main.dust[num3];
