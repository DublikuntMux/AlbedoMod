...
namespace Albedo.Projectiles
{
    public abstract class BaseArena : ModProjectile
    {
        ...
        {
            return Math.Abs((targetHitbox.Center.ToVector2() - projHitbox.Center.ToVector2()).Length() - threshold) <
        ...
        public override void AI()
        {
            ...
            if (val != null)
            {
                ...
                {
                    var num = localPlayer.Distance(projectile.Center);
                    if (num > threshold && num < threshold * 4f)
                    {
                        ...
                        var num2 = vector.Length() - threshold;
                        vector.Normalize();
                        vector *= num2 < 17f ? num2 : 17f;
                        localPlayer.position += vector;
                        for (var i = 0; i < 20; i++)
                        {
                            var num3 = Dust.NewDust(localPlayer.position, localPlayer.width, localPlayer.height,
        ...
        {
            var num = (255f - projectile.alpha) / 255f;
            var num2 = threshold * num;
            var num3 = (int) (300f * num);
            for (var i = 0; i < num3; i++)
            {
                var num4 = Dust.NewDust(projectile.Center, 0, 0, dustType, 0f, 0f, 0, default, 4f);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (var i = 0; i < 32; i++)
            {
                var num2 = (projectile.frame + i) % Main.projFrames[projectile.type];
                var y = num * num2;
                var rectangle = new Rectangle(0, y, texture2D.Width, num);
                var origin = rectangle.Size() / 2f;
                var num3 = (float) Math.PI / 16f * i + projectile.ai[0];
                ...
                for (var j = 0; j < 4; j++)
                {
                    ...
                    var rotation = num3 + projectile.rotation;
                ...
                var rotation2 = num3 + projectile.rotation;
                Main.spriteBatch.Draw(texture2D,
