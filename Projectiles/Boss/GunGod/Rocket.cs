...
namespace Albedo.Projectiles.Boss.GunGod
{
    public class Rocket : ModProjectile
    {
        ...
        public override void AI()
        {
            if (projectile.ai[1] > 0f)
            {
                ...
                if ((projectile.ai[1] -= 1f) == 0f)
                {
                    ...
                    for (var i = 0; i < 36; i++)
                    {
                        ...
                        var num = Dust.NewDust(projectile.Center, 0, 0, 228);
            ...
            else
            {
                if (projectile.ai[0] >= 0f && projectile.ai[0] < 255f)
                {
                    ...
                    {
                        var num2 = (Main.player[(int) projectile.ai[0]].Center - projectile.Center).ToRotation() -
                ...
                if ((projectile.localAI[0] += 1f) > 10f)
                {
                    ...
                    for (var j = 0; j < 36; j++)
                    {
                        ...
                        var num3 = Dust.NewDust(projectile.Center, 0, 0, 228);
            ...
            var num4 = Dust.NewDust(projectile.Center, 0, 0, 228);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            for (var j = 0; j < 40; j++)
            {
                var num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 228, 0f, 0f, 0,
                    default, 2.5f);
                Main.dust[num].noGravity = true;
                var obj = Main.dust[num];
                obj.velocity *= 3f;
                var num2 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 228, 0f, 0f, 100,
                    default, 1.5f);
