...
namespace Albedo.Projectiles.Boss.GunGod
{
	public class GunGodSword : BaseDeathray
	{
		...
		public override void AI()
		{
			...
			for (int i = 0; i < array.Length; i++) array[i] = 3000f;
			...
			if (projectile.localAI[0] % 2f == 0f) {
				if (val.velocity != Vector2.Zero && --Counter < 0) {
					...
					if (Main.netMode != NetmodeID.MultiplayerClient) {
						...
						var vector2 = projectile.velocity.RotatedBy(Math.PI / 2.0 * Math.Sign(projectile.ai[0]));
						for (int k = 1; k <= 15; k++) {
							center += projectile.velocity * 3000f / 15f;
							Projectile.NewProjectile(center, vector2, ModContent.ProjectileType<Sickle2>(),
				...
				for (int l = 0; l < 2; l++) {
					int num5 = Dust.NewDust(projectile.position + projectile.velocity * Main.rand.NextFloat(3000f),
						projectile.width, projectile.height, 87, 0f, 0f, 0, Color.White, 6f);
					Main.dust[num5].noGravity = true;
					var obj = Main.dust[num5];
					obj.velocity *= 4f;
				}
				...
				if (++projectile1.frameCounter > 3) {
					...
					if (++projectile2.frame > 10) projectile.frame = 0;
				}
			}
			...
			if (!SpawnedHandle) {
				...
				if (Main.netMode != NetmodeID.MultiplayerClient) {
					Projectile.NewProjectile(projectile.Center, projectile.velocity,
						ModContent.ProjectileType<GunGodSwordHandle>(), projectile.damage, projectile.knockBack,
						projectile.owner, (float) Math.PI / 2f, projectile.identity);
					Projectile.NewProjectile(projectile.Center, projectile.velocity,
						ModContent.ProjectileType<GunGodSwordHandle>(), projectile.damage, projectile.knockBack,
						projectile.owner, -(float) Math.PI / 2f, projectile.identity);
				}
		...
		public override void OnHitPlayer(Player target, int damage, bool crit)
		{
			...
			Projectile.NewProjectile(target.Center + Main.rand.NextVector2Circular(100f, 100f), Vector2.Zero,
				ModContent.ProjectileType<Blast>(), 0, 0f, projectile.owner);
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			var texture2D = mod.GetTexture("Projectiles/Boss/Deathrays/GunGod/GunGodDeathray_" + projectile.frame);
			var texture2D2 = mod.GetTexture("Projectiles/Boss/Deathrays/GunGod/GunGodDeathray2_" + projectile.frame);
			var texture2D3 = mod.GetTexture("Projectiles/Boss/Deathrays/" + texture + "3");
			float num = projectile.localAI[1];
			var color = new Color(255, 255, 255, 0) * 0.95f;
			var spriteBatch2 = Main.spriteBatch;
			var texture2D4 = texture2D;
			var position = projectile.Center - Main.screenPosition;
			spriteBatch2.Draw(texture2D4, position, null, color, projectile.rotation, texture2D.Size() / 2f,
			...
			if (num > 0f) {
				float num2 = 0f;
				var value = new Rectangle(0, 0, texture2D2.Width, 30);
				while (num2 + 1f < num) {
					if (num - num2 < value.Height) value.Height = (int) (num - num2);
					Main.spriteBatch.Draw(texture2D2, center - Main.screenPosition, value, color, projectile.rotation,
						new Vector2(value.Width / 2, 0f), projectile.scale, SpriteEffects.None, 0f);
					num2 += value.Height * projectile.scale;
					center += projectile.velocity * value.Height * projectile.scale;
					value.Y += 30;
					if (value.Y + value.Height > texture2D2.Height) value.Y = 0;
				}
			}
			...
			spriteBatch3.Draw(texture2D5, position2, null, color, projectile.rotation, texture2D3.Frame().Top(),
				projectile.scale, SpriteEffects.None, 0f);
			return false;
		}
	}
