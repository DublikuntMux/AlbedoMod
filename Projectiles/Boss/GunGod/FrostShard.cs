...
namespace Albedo.Projectiles.Boss.GunGod
{
	public class FrostShard : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int i = 0; i < 3; i++) {
				int num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 76);
		...
		public override void OnHitPlayer(Player target, int damage, bool crit) => target.AddBuff(44, 120);
