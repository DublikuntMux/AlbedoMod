...
namespace Albedo.Projectiles.Boss.GunGod
{
    public class Laser : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            for (var i = 0; i < 4; i++)
            {
                var num = Dust.NewDust(projectile.position, projectile.width, projectile.height, 222);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (var i = 1; i < ProjectileID.Sets.TrailCacheLength[projectile.type]; i++)
