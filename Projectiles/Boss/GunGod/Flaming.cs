...
namespace Albedo.Projectiles.Boss.GunGod
{
	public class Flaming : ModProjectile
	{
		...
		public override void AI()
		{
			...
			float num = projectile.ai[0] < 0f && projectile.ai[1] > 0f
		...
		public override void Kill(int timeLeft)
		{
			...
			float num2 = 12f;
			for (int i = 0; i < num; i++) {
				int num3 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 87, 0f, 0f, 0,
					default, 3.5f);
			...
			for (int j = 0; j < num; j++) {
				int num4 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0f, 0f, 0, default,
					3.5f);
		...
		public override Color? GetAlpha(Color lightColor) =>
			new Color(255, 255, 255, projectile.ai[1] < 0f ? 150 : 255) * projectile.Opacity *
