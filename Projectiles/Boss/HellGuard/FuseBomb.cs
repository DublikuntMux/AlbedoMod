...
namespace Albedo.Projectiles.Boss.HellGuard
{
  ...
  {
    public override void SetStaticDefaults() => DisplayName.SetDefault("Bomb");
    ...
    public override void OnHitPlayer(Player target, int damage, bool crit)
    {
      ...
      if (!AlbedoUtils.BossIsAlive(ref AlbedoGlobalNpc.HellGuard, ModContent.NPCType<NPCs.Boss.HellGuard.HellGuard>()))
        return;
    ...
    public override void Kill(int timeLeft)
    {
      ...
      {
        int index4 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0.0f, 0.0f, 100, new Color(), 3.5f);
        Main.dust[index4].noGravity = true;
        Dust dust1 = Main.dust[index4];
        dust1.velocity *= 7f;
        int index5 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 6, 0.0f, 0.0f, 100, new Color(), 1.5f);
      ...
      for (int index6 = 0; index6 < 3; ++index6)
      {
        ...
        if (index6 == 1)
          num = 0.8f;
        int index7 = Gore.NewGore(projectile.Center, new Vector2(), Main.rand.Next(61, 64), 1f);
        Gore gore1 = Main.gore[index7];
        gore1.velocity *= num;
        ++Main.gore[index7].velocity.X;
        ++Main.gore[index7].velocity.Y;
        int index8 = Gore.NewGore(projectile.Center, new Vector2(), Main.rand.Next(61, 64), 1f);
        Gore gore2 = Main.gore[index8];
        gore2.velocity *= num;
        --Main.gore[index8].velocity.X;
        ++Main.gore[index8].velocity.Y;
        int index9 = Gore.NewGore(projectile.Center, new Vector2(), Main.rand.Next(61, 64), 1f);
        Gore gore3 = Main.gore[index9];
        gore3.velocity *= num;
        ++Main.gore[index9].velocity.X;
        --Main.gore[index9].velocity.Y;
        int index10 = Gore.NewGore(projectile.Center, new Vector2(), Main.rand.Next(61, 64), 1f);
