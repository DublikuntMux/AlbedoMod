  ...
  public class EarthChainBlast : ModProjectile
  {
    ...
    public override void AI()
    {
      ...
      else
      {
        ...
        if (++projectile.localAI[1] == 8.0 && projectile.ai[1] > 0.0 && Main.netMode != NetmodeID.MultiplayerClient)
        {
          --projectile.ai[1];
          Vector2 rotationVector2 = projectile.ai[0].ToRotationVector2();
          ...
          if (projectile.ai[1] > 2.0)
          {
            ...
            {
              if (index != 0)
                Projectile.NewProjectile(projectile.Center + projectile.width * 1.25f * Utils.RotatedBy(rotationVector2, (double) MathHelper.ToRadians(60f) * index + Main.rand.NextFloat(-radians, radians), new Vector2()), Vector2.Zero, projectile.type, projectile.damage, 0.0f, projectile.owner, projectile.ai[0], projectile.ai[1]);
            }
          }
          else
            Projectile.NewProjectile(projectile.Center + projectile.width * 2.25f * Utils.RotatedBy(rotationVector2, (double) Main.rand.NextFloat(-radians, radians), new Vector2()), Vector2.Zero, projectile.type, projectile.damage, 0.0f, projectile.owner, projectile.ai[0], projectile.ai[1]);
        }
    ...
    public override void Kill(int timeLeft)
    {
      ...
      if (!Main.rand.NextBool(4))
        return;
      int index1 = Gore.NewGore(projectile.position + new Vector2(projectile.width * Main.rand.Next(100) / 100f, projectile.height * Main.rand.Next(100) / 100f) - Vector2.One * 10f, new Vector2(), Main.rand.Next(61, 64), 1f);
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
    {
      ...
      Color color = projectile.ai[1] <= 3.0 ? Color.Lerp(new Color((int) byte.MaxValue, 95, 46, 50), new Color(150, 35, 0, 100), (float) ((3.0 - projectile.ai[1]) / 3.0)) : Color.Lerp(new Color((int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue, 0), new Color((int) byte.MaxValue, 95, 46, 50), (float) ((7.0 - projectile.ai[1]) / 4.0));
