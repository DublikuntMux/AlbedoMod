...
namespace Albedo.Projectiles.Boss.HellGuard
{
    public class CrystalBomb : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            var index = Dust.NewDust(projectile.position, projectile.width, projectile.height, 197, 0.0f, 0.0f, 100,
                Color.Transparent);
        ...
        {
            Main.PlaySound(SoundID.Item27, projectile.position);
            for (var index1 = 0; index1 < 40; ++index1)
            {
                var index2 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 68);
            ...
            {
                var num2 = projectile.velocity.Length() * Main.rand.Next(-60, 61) * 0.01f +
                           Main.rand.Next(-20, 21) * 0.4f;
                var num3 = projectile.velocity.Length() * Main.rand.Next(-60, 61) * 0.01f +
                           Main.rand.Next(-20, 21) * 0.4f;
