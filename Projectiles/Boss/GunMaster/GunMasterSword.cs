...
{
	public class GunMasterSword : BaseDeathray
	{
		public int Counter;

		public bool SpawnedHandle;

		public GunMasterSword()
			: base(300f, "GunMasterDeathray")
		{
		}
		...
		public override void AI()
		{
			...
			NPC val = AlbedoUtils.NpcExists(projectile.ai[1], ModContent.NPCType<NPCs.Boss.GunMaster.GunMaster>());
			if (val == null)
			{
				projectile.Kill();
				return;
			...
			if (!SpawnedHandle)
			{
				...
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					Projectile.NewProjectile(projectile.Center, projectile.velocity, ModContent.ProjectileType<GunMasterSwordHandle>(), projectile.damage, projectile.knockBack, projectile.owner, (float)Math.PI / 2f, projectile.identity);
					Projectile.NewProjectile(projectile.Center, projectile.velocity, ModContent.ProjectileType<GunMasterSwordHandle>(), projectile.damage, projectile.knockBack, projectile.owner, -(float)Math.PI / 2f, projectile.identity);
				}
			}
		}
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D = mod.GetTexture("Projectiles/Boss/Deathrays/GunMaster/GunMasterDeathray_" + projectile.frame);
			Texture2D texture2D2 = mod.GetTexture("Projectiles/Boss/Deathrays/GunMaster/GunMasterDeathray2_" + projectile.frame);
