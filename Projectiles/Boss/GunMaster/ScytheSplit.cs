...
namespace Albedo.Projectiles.Boss.GunMaster
{
	public class ScytheSplit : ModProjectile
	{
		...
		{
			int num = projectile.ai[1] >= 0f ? 50 : 25;
			float num2 = projectile.ai[1] >= 0f ? 15 : 6;
			for (int i = 0; i < num; i++)
			{
				int num3 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 70, 0f, 0f, 0, default(Color), 3.5f);
			...
			if (num4 != -1)
			{
				Vector2 vector = projectile.ai[1] == 0f ? Vector2.Normalize(projectile.velocity) : projectile.DirectionTo(Main.player[num4].Center);
				vector *= 30f;
				int num5 = projectile.ai[1] == 0f ? 6 : 10;
				for (int j = 0; j < num5; j++)
				{
					Projectile.NewProjectile(projectile.Center, Utils.RotatedBy(vector, (double)((float)Math.PI * 2f / num5 * j), default(Vector2)), ModContent.ProjectileType<Sickle2>(), projectile.damage, projectile.knockBack, projectile.owner, num4);
				}
			}
		}
