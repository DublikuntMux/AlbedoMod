...
{
	public class GunMasterSwordHandle : BaseDeathray
	{
		public int Counter;

		public GunMasterSwordHandle()
			: base(150f, "GunMasterDeathray")
		{
		}
		...
		public override void AI()
		{
			...
			int byUuidReal = AlbedoUtils.GetByUUIDReal(projectile.owner, (int)projectile.ai[1], ModContent.ProjectileType<GunMasterSword>());
			if (byUuidReal != -1)
			{
				projectile.Center = (Main.projectile[byUuidReal].Center + Main.projectile[byUuidReal].velocity * 75f);
				projectile.velocity = Utils.RotatedBy(Main.projectile[byUuidReal].velocity, (double)this.projectile.ai[0], default(Vector2));
			}
			if (projectile.velocity.HasNaNs() || projectile.velocity == Vector2.Zero)
			{
				projectile.velocity = -Vector2.UnitY;
